...
# --- WORKER 1: YOUTUBE/URL DOWNLOADER ---
class BatchDownloadWorker(QThread):
    progress_signal = Signal(int)
    log_signal = Signal(str)
    finished_signal = Signal()
...
# --- WORKER 2: SPOTIFY DOWNLOADER (THE DUMP) ---
class SpotifyDownloadWorker(QThread):
    ...
    def run(self):
        # spotdl download logic
        cmd = [sys.executable, "-m", "spotdl", "download"] + self.urls
        if self.output_dir:
            cmd += ["--output", self.output_dir]

        self.log_signal.emit(">> INITIALIZING SPOTIFY DUMP PROTOCOL...")
        process = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
            text=True, encoding='utf-8', errors='replace'
        )
...
# --- WORKER 3: STEM SEPARATOR (THE DIP) ---
class BatchStemWorker(QThread):
    log_signal = Signal(str)
    progress_signal = Signal(int)
    finished_signal = Signal()
...
class RipAndDip(QWidget):
    ...
    def setup_ui(self):
        ...
        self.tab_rip = QWidget()
        self.setup_rip_tab()
        self.tabs.addTab(self.tab_rip, " [ RIP ] ")

        # DUMPS Tab
        self.tab_dump = QWidget()
        self.setup_dump_tab()
        self.tabs.addTab(self.tab_dump, " [ DUMPS ] ")

        # DIP Tab
        self.tab_dip = QWidget()
        self.setup_dip_tab()
        self.tabs.addTab(self.tab_dip, " [ DIP ] ") 

        # Log Section
        self.log = QTextEdit()
        self.log.setReadOnly(True)
        self.log.setMaximumHeight(150)
        layout.addWidget(QLabel("SYSTEM_LOG:"))
        layout.addWidget(self.log)

    def setup_rip_tab(self):
        layout = QVBoxLayout(self.tab_rip)
        self.url_input = QPlainTextEdit()
        self.url_input.setPlaceholderText(">> PASTE YOUTUBE/URL LINKS HERE")
        layout.addWidget(self.url_input)
        ...
        self.dl_btn = QPushButton("INITIATE RIP")
        self.dl_btn.clicked.connect(self.start_download)
        layout.addWidget(self.dl_btn)

    def setup_dump_tab(self):
        layout = QVBoxLayout(self.tab_dump)
        self.spotify_input = QPlainTextEdit()
        self.spotify_input.setPlaceholderText(">> PASTE SPOTIFY LINKS HERE")
        layout.addWidget(self.spotify_input)
        
        self.dump_btn = QPushButton("EXECUTE DUMP (SPOTIFY)")
        self.dump_btn.clicked.connect(self.start_spotify_download)
        self.dump_btn.setStyleSheet("border-color: #1DB954; color: #1DB954;")
    ...
    def setup_dip_tab(self):
        ...
        pick_btn = QPushButton("ADD AUDIO FILES")
        pick_btn.clicked.connect(self.choose_source_files)
        layout.addWidget(pick_btn)
        
        self.split_btn = QPushButton("EXECUTE DIP (STEMS)")
        self.split_btn.clicked.connect(self.start_split)
        layout.addWidget(self.split_btn)

    def apply_styles(self):
        # Detecting your custom border.png
        border_css = ""
        if os.path.exists("border.png"):
            border_css = "border-image: url(border.png) 50 stretch; border-width: 50px;"
        else:
            border_css = "border: 2px solid #00f0ff;"

        self.setStyleSheet(f"""
            QFrame#MainFrame {{ {border_css} background-color: #0a0a12; }}
            QWidget {{ color: #00f0ff; font-family: 'Consolas'; background: transparent; }}
            QTabWidget::pane {{ border: 1px solid #005f73; background: #000000; }}
            QTabBar::tab {{ background: #0a0a12; padding: 10px; color: #005f73; border: 1px solid #005f73; }}
            QTabBar::tab:selected {{ background: #00f0ff; color: #000000; font-weight: bold; }}
            QPlainTextEdit, QTextEdit, QListWidget, QComboBox {{ background: #050505; border: 1px solid #005f73; color: #00f0ff; }}
            QPushButton {{ border: 1px solid #00f0ff; padding: 10px; font-weight: bold; background: #0a0a12; }}
            QPushButton:hover {{ background: #00f0ff; color: #000000; }}
            QPushButton#CloseBtn:hover {{ background: #ff003c; color: white; }}
            QProgressBar {{ border: 1px solid #005f73; text-align: center; }}
            QProgressBar::chunk {{ background-color: #00f0ff; }}
        """)

    # Functionality
    def start_download(self):
        urls = [l.strip() for l in self.url_input.toPlainText().split('\n') if l.strip()]
        if not urls: return
        fmt = self.format_box.currentText()
        flags = ["-x", "--audio-format", fmt.lower()] if fmt != "MP4" else []
        self.dl_worker = BatchDownloadWorker(urls, flags, self.dl_output_dir)
        self.dl_worker.log_signal.connect(self.log.append)
        self.dl_worker.progress_signal.connect(self.dl_progress.setValue)
        self.dl_worker.start()

    def start_spotify_download(self):
        urls = [l.strip() for l in self.spotify_input.toPlainText().split('\n') if l.strip()]
        if not urls: return
        self.dump_btn.setEnabled(False)
        self.spot_worker = SpotifyDownloadWorker(urls, self.dl_output_dir)
        self.spot_worker.log_signal.connect(self.log.append)
        self.spot_worker.finished_signal.connect(lambda: self.dump_btn.setEnabled(True))
        self.spot_worker.start()

    def choose_source_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Select Audio", "", "Audio (*.mp3 *.wav *.flac)")
        for f in files:
            self.stem_input_files.append(f)
            self.file_list.addItem(os.path.basename(f))

    def start_split(self):
        if not self.stem_input_files: return
        self.stem_worker = BatchStemWorker(self.stem_input_files, self.stem_output_dir)
        self.stem_worker.log_signal.connect(self.log.append)
        self.stem_worker.start()
...
if __name__ == "__main__":
    ...
    win = RipAndDip()
    win.show()
>>>>>>> 169d893 (Initial commit: Rip and Dip Studio Edition with Spotify support)
    sys.exit(app.exec())